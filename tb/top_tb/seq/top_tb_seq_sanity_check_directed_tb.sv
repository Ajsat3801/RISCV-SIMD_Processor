...
class top_tb_seq_sanity_check_directed_tb extends top_tb_seq_program_base;
    ...
    virtual function void build_program();
        ...
        prog.push_back( rv32i_add(7, 1, 2) );                     // R7  becomes 19
        prog.push_back( rv32i_sub(8, 1, 2) );                     // R8  becomes 11
        prog.push_back( rv32i_addi(9, 8, 5) );                    // R9  becomes 16
        prog.push_back( rv32i_lw(10, 8, 0) );                     // lw   r10, 0(r8)   R10 becomes 627
        prog.push_back( rv32i_lw(11, 1, -1) );                    // lw   r11, -1(r1)  R11 becomes -785
        prog.push_back( rv32m_mul(12, 10, 11) );                  // R12 becomes -492195
        prog.push_back( rv32m_mulh(13, 10, 11) );                 // R13 becomes -1
        prog.push_back( rv32m_mulhu(14, 10, 11) );                // R14 becomes 626
        prog.push_back( rv32m_mulhsu(15, 11, 10) );               // R15 becomes -1
        prog.push_back( rv32i_andi(16, 15, 8) );                  // R16 becomes 8
        prog.push_back( rv32i_slli(17, 2, 2) );                   // R17 becomes 16
        prog.push_back( rv32i_srli(18, 17, 1) );                  // R18 becomes 8
        prog.push_back( rv32m_div(19, 1, 4) );                    // R19 becomes 5
        prog.push_back( rv32m_divu(20, 1, 4) );                   // R20 becomes 5
        prog.push_back( rv32m_rem(21, 1, 4) );                    // R21 becomes 0
        prog.push_back( rv32m_div(22, 3, 4) );                    // R22 becomes -2
        prog.push_back( rv32m_remu(23, 3, 4) );                   // R23 becomes 0
        prog.push_back( rv32m_rem(24, 3, 4) );                    // R24 becomes 1
        prog.push_back( rv32i_slt(25, 3, 1) );                    // R25 becomes 1
        prog.push_back( rv32i_xori(26, 9, 7) );                   // R26 becomes 23
        prog.push_back( rv32i_sltu(27, 1, 2) );                   // R27 becomes 0
        prog.push_back( rv32i_ori(28, 26, 31) );                  // R28 becomes 31
        prog.push_back( rv32i_slti(29, 3, 0) );                   // R29 becomes 1
        prog.push_back( rv32i_sltiu(30, 3, 1) );                  // R30 becomes 0
        prog.push_back( rv32m_mul(31, 1, 2) );                    // R31 becomes 60
        prog.push_back( rv32i_add(25, 10, 11) );                  // R25 becomes -158

        // tests overloading of PRF
        prog.push_back( rv32m_div(27, 17, 15) );                  // R27 becomes -16
        prog.push_back( rv32i_sub(29, 10, 11) );                  // R29 becomes 1412
        ...
        // test vector ops
        prog.push_back( rv32v_vle32_v(17, 1) );                   // V1  becomes [ 1,  2,  3,  4]
        prog.push_back( rv32i_addi(26, 26, 1) );                  // R26 becomes 24
