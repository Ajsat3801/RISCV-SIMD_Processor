...
class lib_fifo_fwft_1push_drv extends uvm_driver #(lib_fifo_fwft_1push_tr);

    virtual lib_fifo_fwft_1push_if vif;
    lib_fifo_fwft_1push_tr tr;

    `uvm_component_utils(lib_fifo_fwft_1push_drv)
    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db #(virtual lib_fifo_fwft_1push_if)::get(this, "", "vif", vif)) begin
            `uvm_fatal("DRV/NOVIF", "virtual interface vif not set for lib_fifo_fwft_1push_drv")
        end
    endfunction
    ...
    task drive_to_interface(lib_fifo_fwft_1push_tr tr);
