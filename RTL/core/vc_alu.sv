...
module vc_alu (
    ...
    input  signal_pkg::vc_ex_input_signal_t valu_input_i,
    output signal_pkg::vc_ex_output_signal_t valu_output_o,
);
...
always_ff @(posedge clk_i) begin
    if(!reset_n) begin
        ...
        valu_output_o.valid   <= (&valid);
        valu_output_o.prf_tag <= valu_input_i.prf_tag;
        valu_output_o.rob_id  <= valu_input_i.rob_id;
        valu_output_o.data    <= valu_output;
    end
end