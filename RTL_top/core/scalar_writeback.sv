...
always @(posedge clk) begin
    ...
    if(!reset_n) begin
        ...
        for(int i=0; i<8; i=i+1) begin
            ...
            salu_wb_queue[i].ROB_id <= 5'b0;
        end
        for(int i=0; i<4; i=i+1) begin
            smuldiv_wb_queue[i].wb_data <= 32'b0;
            smuldiv_wb_queue[i].ROB_id <= 5'b0;
        end
        ...
        if(slsu_result_valid) begin // pass lsu result to register
            wb_data_q.ROB_id <= scalar_lsu_res.ROB_id;
...
assign CDB_data.ROB_id = wb_data_q.ROB_id;
assign CDB_data.data = wb_data_q.wb_data;
assign CDB_data.valid = write_enable_q;
