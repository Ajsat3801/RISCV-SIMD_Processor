...
logic [2:0] salu_q_head, salu_q_tail, salu_q_count;
logic salu_q_full, salu_q_empty; 

wb_desc_t smuldiv_wb_queue[3:0];
logic [1:0] smuldiv_q_head, smuldiv_q_tail, smuldiv_q_count;
...
always @(posedge clk) begin
    if(!resetn) begin
        ...
        for(int i=0; i<8; i=i+1) begin
            ...
            salu_wb_queue[i].rd <= 32'b0;
        end
        for(int i=0; i<4; i=i+1) begin
            smuldiv_wb_queue[i].wb_data <= 32'b0;
            smuldiv_wb_queue[i].rd <= 32'b0;
        end
        ...
        if(slsu_result_valid) begin // pass lsu result to register
            rd_q <= scalar_lsu_res.rd;
            wb_data_q <= scalar_lsu_res.wb_data;
            reset_busy_q <= 1;
            write_enable_q <= 1;
            next_state <= state;
        end
        else begin
            // do round robin between ALU and MULDIV for dequeue
            
            if(state == ALU) begin
                //send ALU
                if(!salu_q_empty) begin
                ...
                if(!smuldiv_q_empty) next_state <= MULDIV;
            end
            
            if(state == MULDIV) begin
                //send MULDIV
                if(!smuldiv_q_empty) begin
                ...
                if(!salu_q_empty) next_state = ALU;
            end
